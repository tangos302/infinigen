...
def _scene_brief_block(map_size: str) -> str:
    ...
    return (
        ...
        "to author a `Composition` that captures the artistic intent of the "
        "prompt — where the cliff is, what shape the plateau has, which "
        "ridge the eye should follow. The procedural pipeline turns your "
        "Composition into a heightmap; without your direction it produces "
        "generic 'noise on a grid' terrain that has no composition at all.\n\n"
        "Think of each primitive (Hero, Mesa, Cliff, Ridge, Pathway, Water, "
        "Hoodoo/Arch/Pillar) as a brush stroke. An art director making a "
        "Sky-CotL-style scene wouldn't reach for noise; they'd reach for "
        "deliberate shapes: a fortress on a polygonal mesa, a coastal cliff "
        "dropping eastward, a river valley winding between two ranges. Pick "
        "the primitive that matches the artistic intent, not the one that's "
        "easiest to copy from the example.\n\n"
        ...
        "```python\n"
        "from infinigen.maquette.runtime.influence import Composition, Hero, Pathway, Ridge, Water, Cliff, Mesa\n"
        "from infinigen.maquette.runtime.landmarks import Hoodoo, Arch, Pillar  # SDF landmarks (optional)\n"
        ...
        "    # Note: palette_preset='desert' auto-activates a dunes layer\n"
        "    # in the heightmap (crossing sinusoid waves) for Wasteland-\n"
        "    # style rolling sand. No explicit dunes flag needed.\n"
        "    composition=Composition(\n"
        "        # **Single dominant ridge** — Sky CotL's spine silhouette.\n"
        "        # ALWAYS emit ONE ridge for outdoor scenes. Multi-ridge\n"
        "        # composition reads as 'PCG demo'; a single sweeping\n"
        "        # backbone curve is what frames the eye. Heroes sit on\n"
        "        # this backbone (post-hero terrain sampling handles the\n"
        "        # plateau-on-ridge stack automatically).\n"
        "        ridges=[\n"
        ...
        "```\n\n"
        "Operator semantics:\n"
        "- **Ridge(waypoints, height=16, width=25, hardness=1.4)** — sweeping "
        "Catmull-Rom-smoothed elevation along a polyline. **EMIT EXACTLY ONE "
        "ridge for outdoor painterly scenes** — this is the scene's "
        "structural backbone, heroes sit on top, and multi-ridge composition "
        "reads as 'PCG demo' rather than designed terrain. ``width`` is the "
        "falloff radius (Gaussian, ~37 % at one width). Default `height=16` "
        "is the right amplitude on our 320 BU world; bump to 22-26 for "
        "explicit-mountain prompts, drop to 10-12 for very flat meadow "
        "prompts. Skip the ridge entirely only when the prompt asks for an "
        "open horizon (open ocean, vast desert with no mountains).\n"
        "- **Hero(cx, cy, radius, hardness=1.2, lift=0.0, mode='flatten')** — "
        "Gaussian-falloff plateau centered at (cx, cy). For the painterly "
        "Sky-CotL look, prefer **broad soft plateaus**: `radius=14–22 BU`, "
        "`hardness=1.0–1.4`. Sharp `hardness=2.5+` is reserved for "
        "fortress/mesa scenes that explicitly call for cliff edges. "
        "**`mode='dome'`** is the painterly alternative: instead of "
        "destructively flattening the terrain, it ADDS a Gaussian dome of "
        "height `lift` on top of existing terrain. The hero's building "
        "sits ON the dome (which integrates softly with surrounding "
        "terrain) instead of inside a carved mesa. Use `mode='dome'` for "
        "natural-feeling rises (`lift=2–4`) and `mode='flatten'` only "
        "when you want a deliberate flat shelf. `lift` (flatten mode) "
        "raises the plateau above local terrain by N BU; cap "
        "`lift / radius < 0.4` to avoid Gaussian-splat reading.\n"
        ...
        "**`depth=0.0` is the painterly default — colour-only path, no "
        "trench.** `depth=-0.05` gives a worn track (use only for explicit "
        "sunken roads); `depth=0.0` is a level cobble "
        ...
        "primary.\n"
        "- **Cliff(start, end, height=8, blend=3, facing='right', "
        "hard_top=None)** — directional drop along the line ``start → end``. "
        "Walking start → end, the side picked by ``facing`` (``right`` or "
        "``left``) drops by ``height`` BU over a ``blend`` BU smoothstep. "
        "Use this when the prompt names a deliberate cliff edge, plateau "
        "drop, or coastline. ``hard_top=12.0`` clips the high side to a "
        "constant elevation — pair with a Mesa for a clean plateau wall. "
        "This is how an art director draws a cliff: a chosen line and "
        "direction, not a procedural artifact.\n"
        "- **Mesa(polygon, target_z=None, lift=0.0, blend=3.0)** — "
        "flat-topped landmark with a polygon footprint. Use when you want "
        "the silhouette of the top deck under your control (fortress base, "
        "monastery shelf, designed plateau, sandstone butte). ``polygon`` "
        "is a closed list of (x, y) points — minimum 3 vertices, order "
        "doesn't matter. Inside the polygon the terrain snaps to "
        "``target_z`` (sampled at centroid + ``lift`` if None). Outside, "
        "smoothsteps over ``blend`` BU back to surrounding terrain. "
        "Use Mesa instead of Hero when the desk shape matters — an "
        "L-shape, a trapezoidal cliff terrace, an irregular butte top.\n"
        ...
        "- **Water(cx, cy, radius, depth=2.0)** — round basin centered at "
        "(cx, cy). The basin floor sits `depth` BU below the natural "
        "surface. **Author depth ≥ 2.0 BU for any visible pool** — "
        "shallower basins read as a flat painted disc on the sand "
        "(the water surface ends up flush with surrounding terrain). "
        "Oasis pools, lake basins, courtyard ponds: depth 2–4 BU. "
        "The runtime auto-clamps to a 1.5 BU minimum so you can't "
        "ship a puddle by accident.\n"
        "- **Don't author a flatten Hero on top of a Water primitive.** "
        "A `Hero(mode='flatten', radius=R)` covering the same XY as "
        "`Water(cx, cy, ...)` wipes any pre-erosion trough you placed "
        "and forces the basin floor to natural terrain height — the "
        "only depression left is whatever Water carves. If you want "
        "the oasis sunk into a flat shelf, either (a) drop the hero "
        "and let Water do all the carving (depth 3–4 BU), or (b) keep "
        "the hero with `lift=-2.5` so the shelf itself is depressed.\n\n"
        "Hard rules:\n"
        ...
        "for boardwalks.\n"
        "- **Dress** by calling `dress_path(composition, archetype=..., "
        "terrain=terrain)` once per Composition AFTER `apply_composition` "
        "(import from `infinigen.maquette.runtime.dress_path`). Pass "
        "`archetype='lantern_posts'` for stone_road/town/temple paths, "
        "`'curb_stones'` for dirt_trail/wilderness paths, `'fence_posts'` "
        "for farm/pasture paths. Default cadences (6 / 2 / 4 BU) are tuned; "
        "do NOT pass `spacing` unless the prompt asks for unusual density. "
        "Cluster barrels/crates at junctions manually as before.\n"
