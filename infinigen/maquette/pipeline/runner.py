...
def _scene_brief_block(map_size: str) -> str:
    ...
    return (
        ...
        "        # Optional: a sweeping primary ridge — Sky CotL's spine\n"
        ...
        "```\n\n"
        "Operator semantics:\n"
        "- **Ridge(waypoints, height=8, width=25, hardness=1.4)** — sweeping "
        "Catmull-Rom-smoothed elevation along a polyline. Use ONE ridge as "
        "the scene's structural backbone; heroes sit on top. ``width`` "
        "is the falloff radius (Gaussian, ~37 % at one width). Skip when "
        "the prompt asks for an open meadow with no horizon ridge.\n"
